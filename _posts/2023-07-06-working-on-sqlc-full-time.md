...
to share that we've incorporated our company, Riza[^1], and will be working on
sqlc full-time.
...
database server. This functionality will expande over time, but for now it
powers the built-in `sqlc/db-prepare` rule.

When a
[database](https://docs.sqlc.dev/en/v1.19.0/reference/config.html#database) is
...
Lastly, to keep up with future sqlc developments you can [subscribe to our
newsletter][3].

Riza is a for-profit corporation, so we’ll be exploring monetization strategies
to sustain sqlc development. That being said, we have no intention or plans to
change the license of sqlc. We expect to build adjacent services that enhance
